   ...
   // xhr.setRequestHeader("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type, Accept");
	...
	xhr.send(floderdata);*/
		...
		@foreach ($attachment as $rec)
		 			attachdata.push( [ '{{$loop->iteration}}', '{{$rec->at_filename}}','{{$rec->attachment}}', '{{$rec->at_detail}}',
	...
	xhr.addEventListener("readystatechange", function () {
		if(this.readyState === 4) {
           ...
           var parsed = JSON.parse(resdata);
			...
			var add = parsed[0].added;
			console.log(add[0].hash); // logs "b1"
				...
				$.ajax({
			        ...
			        data:{data:added_data,data_removed:data_removed,type:type,propid:0},
				...
				var floderdata1 = new FormData();
				floderdata1.append("cmd", "mkdir");
				floderdata1.append("target", "l1_"+hashtermname);
				floderdata1.append("name", "{{$accountnumber}}");
