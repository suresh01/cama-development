	...
	xhr.send(floderdata);*/
		...
		@foreach ($attachment as $rec)
 			attachdata.push( [ '{{$loop->iteration}}', '<a href="http://{{$serverhost}}/downloadfile?id={{$rec->at_id}}&Name={{$rec->at_filename}}&path={{$rec->at_path}}\\{{$rec->at_filename}}">{{$rec->at_filename}}</a>','{{$rec->attachment}}', '{{$rec->at_detail}}',
 				'<span><a onclick="" class="action-icons c-delete deleteattachrow" href="#" title="delete">Delete</a></span>','noaction','{{$rec->at_id}}','{{$rec->at_oringinalfilename}}' ] );
 		@endforeach
					...
					buttons: [
						{type: 'button pink', text: 'Delete', click: function($noty) {
					  			...
					  			var floderdata = new FormData();
									...
									xhr.send(floderdata);
									window.location.assign('datasearchdetail?prop_id={{$prop_id}}');
								$noty.close();
						  	}
						},
...
function addAttachment(){
	...
	var fileinputlimit = document.getElementById('filepath');
var FileSize = fileinputlimit.files[0].size / 1024 / 1024; // in MB
	if (FileSize > 2) {
