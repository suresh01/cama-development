...
<title>{{__('objection.Agenda_Property_Detail')}}</title>
@include('includes.header-popup')
	...
	<div id="content">
		<div class="grid_container">		
          ...
          <a href="#" id="" onclick="deleteProperty()" title="Delete Selected"><span>{{__('common.Delete')}} </span></a> 
        </div>

         <div style="float:right;margin-right: 10px;"  class="btn_24_blue">   
          @include('objection.search.newsearch',['tableid'=>'baskettbl', 'action' => 'objectionbaskettable?', 'searchid' => '34'])
...
<script type="text/javascript">
   ...
   function submitForm(){
    ...
    xhr = $.ajax({
            url: 'objectiondetailtable?id={{$id}}&test=manual&ts_='+timestamp,
            type: 'GET',
   ...
   // Array holding selected row IDs
    ...
    $('#baskettbl').DataTable({
      "processing": false,
            ...
            "ajax": {
                "type": "GET",
                "url": 'objectiondetailtable?id={{$id}}',
           ...
           // ajax: '{{ url("inspectionproperty") }}',
            ...
            "columns": [
              {"data": "agd_id", "name": "account number"},
              {"data": null, "name": "sno"},
              {"data": "vd_accno", "name": "account number"},
              {"data": "va_name", "name": "zone"},
              {"data": "ag_siries", "name": "subzone"},
              {"data": "ob_listyear", "name": "address"}
          ],
...
function deleteINProperty(param){
    var noty_id = noty({
          layout : 'center',
                 ...
                 var id= "{{$id}}";
                  var type = "delete";
                     $.ajax({
                       type:'GET',
                       url:'agendadetailtrn',
                       data:{accounts:param,id:id,type:type},
...
function deleteProperty() {
                 ...
                 var id= "{{$id}}";
                  var type = "delete";
                     $.ajax({
                       ...
                       success:function(data){           
                          alert(data.newcount + " Property Deleted");
                          window.location.assign('objectiondetail?term={{$term}}&id={{$id}}');
                       }
                  });
   ...
   function deleteAgendaDetail(id){
