...
<div style="margin-top: 0px;" id="container">
	...
	<div id="content">
		...
		<div id="tenant_table" class="grid_12">	
          ...
          <a href="#" id="" onclick="openTenantUser()" class=""><span>Add New Tenant </span></a>
          ...
          // $noty = $noty element
            var tenantdata = {};
              $('#tenantform').serializeArray().map(function(x){tenantdata[x.name] = x.value;});
                ...
                window.location.assign('tenanttrn?type=insadd&jsondata='+tenantjson);
          $noty.close();
...
$(document).ready(function (){
   ...
   var rows_selected = [];
   	@foreach ($property as $rec)
		 			rows_selected.push( [ '{{ $rec->te_id }}','{{$loop->iteration}}', '{{ $rec->applntype }}', '{{ $rec->tenanttype }}', '{{ $rec->te_no }}','{{ $rec->te_name}}', '{{ $rec->te_addr_ln1 }}', '{{ $rec->te_addr_ln2 }}', '{{ $rec->te_addr_ln3 }}', '{{ $rec->te_addr_ln4 }}','{{ $rec->state }}', '{{ $rec->te_postcode }}','<span><a class="action-icons c-edit" onclick="editTenantUser({{ $rec->te_id }})" href="#" title="Edit">Edit</a></span>'] );
          $('#hiddenvalues').append('input type="hidden" id="applntype_{{ $rec->te_id }}" value="{{ $rec->te_applntype_id }}">'+
  ...
  function CloseMySelf(sender) {
    ...
    if (duplicate === 1 || duplicate === 0){
      window.opener.tenantParent(table);
