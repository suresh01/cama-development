...
FROM `cm_appln_val_tax` where vt_vd_id = ifnull("'.$prop_id.'",0)');
    ...
    public function testBasket(Request $request){
        ...
        order by va_id desc");
         ...
         $propcount = DB::select('select  count(*) totproperty_count from cm_appln_valdetl 
        inner join cm_appln_val on va_id = vd_va_id where  va_vt_id = ifnull("'.$basketid.'",0)');
        ...
        $inspropcount = DB::select('select  count(*) inscount from cm_appln_valdetl 
        inner join cm_appln_val on va_id = vd_va_id
        where vd_approvalstatus_id in ("06","07","08","09","10","11","12") and  va_vt_id = ifnull("'.$basketid.'",0)');

        $valpropcount = DB::select('select  count(*) valcount from cm_appln_valdetl 
        inner join cm_appln_val on va_id = vd_va_id where vd_approvalstatus_id in ("08","09","10","11","12") and  va_vt_id = ifnull("'.$basketid.'",0)');

        App::setlocale(session()->get('locale'));
        
        return view("group.basket_test")->with(array('group'=> $group, 'basketid' => $basketid,'propcount'=>$propcount,'bldgcount'=>$bldgcount,'inspropcount'=>$inspropcount,'valpropcount'=>$valpropcount));
    }
...
on state.tdi_key = rg_applntstate_id     '. $filterquery);
 ...
 to_telno, to_ownname, to_ownno, TO_ADDR_LN1,TO_ADDR_LN2,TO_ADDR_LN3,TO_ADDR_LN4,TO_POSTCODE, TO_STATE_ID, TO_CITY, cm_remisi_reg.*
