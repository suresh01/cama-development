...
var elFinder = function(elm, opts, bootCallback) {
	...
	(function(){
		...
		if (self.isCORS) {
			...
			self.options.requestType = 'get';
			//self.options.customHeaders['X-Requested-With'] = 'XMLHttpRequest';
			//self.options.xhrFields['withCredentials'] = false;
		}
	})();
...
elFinder.prototype = {
	...
	reloadContents : function(url) {
		var dfd = $.Deferred(),
			ifm;
		try {
			ifm = $('<iframe width="1" height="1" scrolling="no" frameborder="no" style="position:absolute; top:-1px; left:-1px" crossorigin="">')
				.attr('src', url)
				.one('load', function() {
					var ifm = $(this);
					try {
...
elFinder.prototype._options = {
	...
	contextmenu : {
		...
		cwd    : ['undo', 'redo', '|', 'back', 'up', 'reload', '|', 'upload', 'mkdir', '|', 'empty', 'hide', '|', 'view', 'sort', 'selectall', 'colwidth', '|', 'places', 'info', 'chmod'],
		...
		var self  = this,
			...
			open = function(file, id) {
				var isJpeg   = (file.mime === 'image/jpeg'),
					...
					sizeImg = quality? $('<img>').attr('crossorigin', fm.isCORS? '' : '').attr('src', canvSrc).on('load', function() {
						try {
