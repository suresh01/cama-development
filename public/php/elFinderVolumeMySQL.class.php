...
class elFinderVolumeMySQL extends elFinderVolumeDriver {
	...
	protected function make($path, $name, $mime) {
		$sql = 'INSERT INTO %s (`parent_id`, `name`, `size`, `mtime`, `mime`, `content`, `read`, `write`, `locked`, `hidden`) VALUES ("%s", "%s", 0, %d, "%s", "", "%d", "%d", "%d", "%d")';
		$sql = sprintf($sql, $this->tbf, $path, $this->db->real_escape_string($name), time(), $mime, 1, 1, 2, 2);
		// echo $sql;
	...
	protected function rootparameter($alias, $read, $write, $locked, $hidden) {
		...
		if($locked !== NULL || $locked !== ''){
			$locked ==true?$locked="1":$locked="2";
			$sql = 'UPDATE `elfinder_file` set `locked` = '.$locked;		
		} else {
			$sql = 'UPDATE `elfinder_file` set `locked` = 2';		
		}		
		$res = $this->db->query($sql);
		
		if($hidden !== NULL || $hidden !== ''){
			$hidden ==true?$hidden="1":$hidden="2";
			$sql = 'UPDATE `elfinder_file` set `hidden` = '.$hidden;		
		} else {
			$sql = 'UPDATE `elfinder_file` set `hidden` = 2';		
		}		
		$res = $this->db->query($sql);
			...
			foreach ($this->options['attributes'] as $a) {
				if (!empty($a['pattern']) || count($a) > 1) {
					...
					if(count(preg_split($pattern, $str, -1, PREG_SPLIT_NO_EMPTY))==0){
						...
						$sql = 'UPDATE `elfinder_file` set `read` = "'.$read.'", `write` = "'.$write.'", `locked` = "2", `hidden` = "2" where  parent_id="'.$parent_id.'"';
						$res = $this->db->query($sql);						
